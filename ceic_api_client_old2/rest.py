...
class RESTClientObject(object):
    ...
    def __init__(self, configuration, pools_size=4, maxsize=None):
        """
        Constructor
        :param configuration: The Configuration class instance
        :param pools_size: Pools size
        :param maxsize: The number of requests to host that are allowed in parallel
        """

        self._configuration = configuration
        self._pools_size = pools_size
        self._maxsize = maxsize
        self._pool_args = self._build_pool_args()
    ...
    def _build_pool_args(self):
        pool_args = self._get_initial_args()
        self._try_set_assert_hostname_arg(pool_args)
        self._try_set_proxy_args(pool_args)

        return pool_args

    def _get_pool_manager(self):
        pool_manager = urllib3.ProxyManager if self._configuration.proxy_url else urllib3.PoolManager
        pool_manager = pool_manager(**self._pool_args)

        return pool_manager

    def _get_initial_args(self):
        return {
            "num_pools": self._pools_size,
            "maxsize": self._get_maxsize_arg(),
            "cert_reqs": self._get_cert_req_arg(),
            "ca_certs": self._get_ca_cert_arg(),
            "key_file": self._configuration.key_file
        }

    def _try_set_assert_hostname_arg(self, pool_args):
        if self._configuration.assert_hostname is not None:
            pool_args.update({"assert_hostname": self._configuration.assert_hostname})

    def _try_set_proxy_args(self, pool_args):
        if self._configuration.proxy_url:
            pool_args.update({"proxy_url": self._configuration.proxy_url})
            proxy_headers = self._get_proxy_headers()
            if proxy_headers is not None:
                pool_args.update({"proxy_headers": proxy_headers})

    def _get_maxsize_arg(self):
        if self._maxsize is None:
            if self._configuration.connection_pool_maxsize is not None:
                return self._configuration.connection_pool_maxsize
            else:
                return self._DEFAULT_MAXSIZE

        return self._maxsize

    def _get_cert_req_arg(self):
        return ssl.CERT_REQUIRED if self._configuration.verify_ssl else ssl.CERT_NONE

    def _get_ca_cert_arg(self):
        return self._configuration.ssl_ca_cert if self._configuration.ssl_ca_cert else certifi.where()

    def _get_proxy_headers(self):
        if self._configuration.proxy_username is None and self._configuration.proxy_password is None:
            return None

        proxy_basic_auth = "{0}:{1}".format(
            self._configuration.proxy_username if self._configuration.username is not None else "",
            self._configuration.proxy_password if self._configuration.password is not None else ""
        )
